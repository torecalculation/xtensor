...
namespace xt {
  ...
  namespace extension {
    template <class Tag, class CT, class... S> struct xview_base_impl;

    template <class CT, class... S> struct xview_base_impl<xtensor_expression_tag, CT, S...> {
      using type = xtensor_empty_base;
    };

    template <class CT, class... S> struct xview_base
        : xview_base_impl<xexpression_tag_t<CT>, CT, S...> {};

    template <class CT, class... S> using xview_base_t = typename xview_base<CT, S...>::type;
  }  // namespace extension
  ...
  template <bool is_const, class CT, class... S> class xview_stepper;

  template <class ST, class... S> struct xview_shape_type;

  namespace detail {

    template <class T> struct is_xrange : std::false_type {};

    template <class T> struct is_xrange<xrange<T>> : std::true_type {};

    template <class S> struct is_xall_slice : std::false_type {};

    template <class T> struct is_xall_slice<xall<T>> : std::true_type {};
    ...
    template <class T> struct static_dimension {
      static constexpr std::ptrdiff_t value = -1;
    };

    template <class T, std::size_t N> struct static_dimension<std::array<T, N>> {
      static constexpr std::ptrdiff_t value = static_cast<std::ptrdiff_t>(N);
    };

    template <class T, std::size_t N> struct static_dimension<xt::const_array<T, N>> {
      static constexpr std::ptrdiff_t value = static_cast<std::ptrdiff_t>(N);
    };

    template <std::size_t... I> struct static_dimension<xt::fixed_shape<I...>> {
    ...
    template <class CT, class... S> struct is_xscalar_impl<xview<CT, S...>> {
    ...
    template <class S> struct is_strided_slice_impl : std::true_type {};

    template <class T> struct is_strided_slice_impl<xkeep_slice<T>> : std::false_type {};

    template <class T> struct is_strided_slice_impl<xdrop_slice<T>> : std::false_type {};

    // If we have no discontiguous slices, we can calculate strides for this view.
    template <class E, class... S> struct is_strided_view
        : std::integral_constant<
              bool, xtl::conjunction<has_data_interface<E>,
    ...
    struct is_contiguous_view_impl<layout_type::row_major, valid, all_seen, range_seen, V> {
      ...
      static constexpr bool is_valid
          = valid
            && (have_all_seen ? is_all_slice : (!range_seen && (is_int_slice || is_range_slice)));
    ...
    struct is_contiguous_view_impl<layout_type::row_major, valid, all_seen, range_seen,
                                   xtl::mpl::vector<>> {
    ...
    struct is_contiguous_view_impl<layout_type::column_major, valid, int_seen, range_seen, V> {
      ...
      static constexpr bool is_valid
          = valid
            && (have_int_seen ? is_int_slice : (!range_seen && (is_all_slice || is_range_slice)));
    ...
    struct is_contiguous_view_impl<layout_type::column_major, valid, int_seen, range_seen,
                                   xtl::mpl::vector<>> {
      static constexpr bool value = valid;
    };

    // TODO relax has_data_interface constraint here!
    template <class E, class... S> struct is_contiguous_view
        : std::integral_constant<
              bool,
              has_data_interface<E>::value
                  && !(E::static_layout == layout_type::column_major
                       && static_cast<std::size_t>(static_dimension<typename E::shape_type>::value)
                              != sizeof...(S))
                  && is_contiguous_view_impl<E::static_layout, true, false, false,
                                             xtl::mpl::vector<S...>>::value> {};

    template <layout_type L, class T, std::ptrdiff_t offset> struct unwrap_offset_container {
      using type = void;
    };
    ...
    template <class T, std::ptrdiff_t start, std::ptrdiff_t end, std::ptrdiff_t offset>
    struct unwrap_offset_container<layout_type::column_major, sequence_view<T, start, end>,
                                   offset> {
      using type = sequence_view<T, start, end - offset>;
    };

    template <class E, class... S> struct get_contigous_shape_type {
      ...
      using type = std::conditional_t<
          ...
          // In the false branch we know that we have only integers at the front OR end, and NO
          // range
          typename unwrap_offset_container<E::static_layout, typename E::inner_shape_type,
                                           integral_count<S...>()>::type>;
    };

    template <class T> struct is_sequence_view : std::integral_constant<bool, false> {};
  ...
  template <class CT, class... S> struct xcontainer_inner_types<xview<CT, S...>> {
    ...
    using extract_storage_type
        = xtl::mpl::eval_if_t<has_data_interface<xexpression_type>,
                              detail::expr_storage_type<xexpression_type>, make_invalid_type<>>;
    using storage_type
        = std::conditional_t<is_const, const extract_storage_type, extract_storage_type>;
  };

  template <class CT, class... S> struct xiterable_inner_types<xview<CT, S...>> {
    using xexpression_type = std::decay_t<CT>;
    ...
    using inner_shape_type = std::conditional_t<
        is_contiguous_view, typename detail::get_contigous_shape_type<xexpression_type, S...>::type,
        typename xview_shape_type<typename xexpression_type::shape_type, S...>::type>;

    using stepper = std::conditional_t<
        is_strided_view, xstepper<xview<CT, S...>>,
        xview_stepper<std::is_const<std::remove_reference_t<CT>>::value, CT, S...>>;

    using const_stepper = std::conditional_t<is_strided_view, xstepper<const xview<CT, S...>>,
  ...
  template <class CT, class... S> class xview
      : public xview_semantic<xview<CT, S...>>,
        public std::conditional_t<detail::is_contiguous_view<std::decay_t<CT>, S...>::value,
  ...
  public:
    ...
    using pointer = std::conditional_t<is_const, typename xexpression_type::const_pointer,
    ...
    using inner_strides_type
        = std::conditional_t<has_trivial_strides,
    ...
    using inner_backstrides_type
        = std::conditional_t<has_trivial_strides,
                             typename detail::unwrap_offset_container<
                                 xexpression_type::static_layout,
                                 xexpression_inner_backstrides_type, integral_count<S...>()>::type,
                             get_strides_t<shape_type>>;
    ...
    using storage_iterator = std::conditional_t<
        has_data_interface<xexpression_type>::value && is_strided_view,
        std::conditional_t<is_const, typename xexpression_type::const_storage_iterator,
    ...
    template <class... Args> reference unchecked(Args... args);
    template <class It> reference element(It first, It last);

    template <class... Args> const_reference operator()(Args... args) const;
    template <class... Args> const_reference unchecked(Args... args) const;
    template <class It> const_reference element(It first, It last) const;

    xexpression_type& expression() noexcept;
    const xexpression_type& expression() const noexcept;

    template <class ST> bool broadcast_shape(ST& shape, bool reuse_cache = false) const;

    template <class ST> bool has_linear_assign(const ST& strides) const;
    ...
    template <class It> inline It data_xbegin_impl(It begin) const noexcept;
    ...
    template <class E> using rebind_t = xview<E, S...>;

    template <class E> rebind_t<E> build_view(E&& e) const;

    //
    // SIMD interface
    //

    template <class requested_type> using simd_return_type
        = xt_simd::simd_return_type<value_type, requested_type>;

    template <class T, class R> using enable_simd_interface
    ...
    template <class T = xexpression_type> enable_simd_interface<T, reference> flat(size_type i);
  ...
  private:
    ...
    template <std::size_t I> struct lesser_condition {
    ...
    template <class... Args> auto make_index_sequence(Args... args) const noexcept;
    ...
    template <class Arg, class... Args> reference access(Arg arg, Args... args);

    const_reference access() const;

    template <class Arg, class... Args> const_reference access(Arg arg, Args... args) const;
    ...
    template <class It> base_index_type make_index(It first, It last) const;
    ...
    template <std::size_t... I> auto compute_strides_impl(std::index_sequence<I...>) const noexcept;
  ...
  template <class E, class... S> auto view(E&& e, S&&... slices);

  template <class E> auto row(E&& e, std::ptrdiff_t index);

  template <class E> auto col(E&& e, std::ptrdiff_t index);
  ...
  namespace detail {
    template <class V> struct get_stepper_impl {
      using xexpression_type = typename V::xexpression_type;
      using type = typename xexpression_type::stepper;
    };

    template <class V> struct get_stepper_impl<const V> {
      using xexpression_type = typename V::xexpression_type;
      using type = typename xexpression_type::const_stepper;
    };
  }  // namespace detail

  template <class V> using get_stepper = typename detail::get_stepper_impl<V>::type;
  ...
  public:
    ...
    xview_stepper(view_type* view, substepper_type it, size_type offset, bool end = false,
  ...
  private:
    ...
    template <class F> void common_step_forward(size_type dim, F f);
    template <class F> void common_step_backward(size_type dim, F f);

    template <class F> void common_step_forward(size_type dim, size_type n, F f);
    template <class F> void common_step_backward(size_type dim, size_type n, F f);

    template <class F> void common_reset(size_type dim, F f, bool backwards);
  ...
  template <class I, std::size_t L, class... S> struct xview_shape_type<std::array<I, L>, S...> {
    using type = std::array<I, L - integral_count<S...>() + newaxis_count<S...>()>;
  };

  template <std::size_t... I, class... S> struct xview_shape_type<fixed_shape<I...>, S...> {
   ...
   * @sa view
   */
  template <class CT, class... S> template <class CTA, class FSL, class... SL>
  xview<CT, S...>::xview(CTA&& e, FSL&& first_slice, SL&&... slices) noexcept
      : xview(std::integral_constant<bool, has_trivial_strides>{}, std::forward<CTA>(e),
              std::forward<FSL>(first_slice), std::forward<SL>(slices)...) {}
  ...
  template <class CT, class... S> inline auto xview<CT, S...>::operator=(const xview& rhs)
      -> self_type& {
   ...
   */
  template <class CT, class... S> template <class E>
   ...
   */
  template <class CT, class... S> inline auto xview<CT, S...>::shape() const noexcept
   ...
   */
  template <class CT, class... S> inline auto xview<CT, S...>::slices() const noexcept
      -> const slice_type& {
    return m_slices;
  }
   ...
   * Returns the slices of the view.
   */
  template <class CT, class... S> inline layout_type xview<CT, S...>::layout() const noexcept {
    return xtl::mpl::static_if<is_strided_view>(
        [&](auto self) {
          if (static_layout != layout_type::dynamic) {
            return static_layout;
          } else {
            bool strides_match = do_strides_match(self(this)->shape(), self(this)->strides(),
                                                  self(this)->m_e.layout(), true);
  ...
  template <class CT, class... S> inline bool xview<CT, S...>::is_contiguous() const noexcept {
   ...
   */
  template <class CT, class... S> template <class T>
   ...
   */
  template <class CT, class... S> template <class... Args>
  inline auto xview<CT, S...>::operator()(Args... args) -> reference {
    ...
    // integers, leading to warning about signed/unsigned conversions in the deeper layers of the
    // access methods
  ...
  template <class CT, class... S> template <class It>
   ...
   */
  template <class CT, class... S> inline auto xview<CT, S...>::expression() noexcept
      -> xexpression_type& {
    return m_e;
  }
   ...
   */
  template <class CT, class... S> inline auto xview<CT, S...>::expression() const noexcept
   ...
   */
  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::storage()
      -> std::enable_if_t<has_data_interface<T>::value, storage_type&> {
    return m_e.storage();
  }

  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::storage() const
      -> std::enable_if_t<has_data_interface<T>::value, const storage_type&> {
    return m_e.storage();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_begin()
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, storage_iterator> {
    return m_e.storage().begin() + data_offset();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_end()
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, storage_iterator> {
    return m_e.storage().begin() + data_offset() + this->size();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_begin() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, const_storage_iterator> {
    return storage_cbegin();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_end() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, const_storage_iterator> {
    return storage_cend();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_cbegin() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, const_storage_iterator> {
    return m_e.storage().cbegin() + data_offset();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_cend() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, const_storage_iterator> {
    return m_e.storage().cbegin() + data_offset() + this->size();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_rbegin()
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          reverse_storage_iterator> {
    return reverse_storage_iterator(storage_end());
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_rend()
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          reverse_storage_iterator> {
    return reverse_storage_iterator(storage_begin());
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_rbegin() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          const_reverse_storage_iterator> {
    return storage_crbegin();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_rend() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          const_reverse_storage_iterator> {
    return storage_crend();
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_crbegin() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          const_reverse_storage_iterator> {
    return const_reverse_storage_iterator(storage_end());
  }

  template <class CT, class... S> template <class T> auto xview<CT, S...>::storage_crend() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
                          const_reverse_storage_iterator> {
   ...
   */
  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::strides() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view,
   ...
   */
  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::data() const
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, const_pointer> {
    return m_e.data();
  }

  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::data()
      -> std::enable_if_t<has_data_interface<T>::value && is_strided_view, pointer> {
    return m_e.data();
  }

  template <class CT, class... S> template <std::size_t... I>
  ...
  template <class CT, class... S> inline auto xview<CT, S...>::underlying_size(size_type dim) const
      -> size_type {
   ...
   */
  template <class CT, class... S> template <class ST>
  ...
  template <class CT, class... S> template <class It>
  inline It xview<CT, S...>::data_xend_impl(It begin, layout_type l,
                                            size_type offset) const noexcept {
    return strided_data_end(*this, begin, l, offset);
  }

  template <class CT, class... S> inline auto xview<CT, S...>::data_xbegin() noexcept
      -> container_iterator {
    return data_xbegin_impl(data());
  }

  template <class CT, class... S> inline auto xview<CT, S...>::data_xbegin() const noexcept
  ...
  template <class CT, class... S> template <class E, class T, class>
  void xview<CT, S...>::assign_to(xexpression<E>& e, bool force_resize) const {
    auto& de = e.derived_cast();
    de.resize(shape(), force_resize);
    std::copy(data() + data_offset(), data() + data_offset() + de.size(),
              de.template begin<static_layout>());
  }

  template <class CT, class... S> template <class E, std::size_t... I>
  ...
  template <class CT, class... S> template <class align, class simd, class T>
  ...
  template <class CT, class... S> template <class T> inline auto xview<CT, S...>::flat(size_type i)
      -> enable_simd_interface<T, reference> {
    XTENSOR_ASSERT(is_contiguous());
    return m_e.flat(data_offset() + i);
  }

  template <class CT, class... S> template <class T>
  inline auto xview<CT, S...>::flat(size_type i) const
      -> enable_simd_interface<T, const_reference> {
    XTENSOR_ASSERT(is_contiguous());
    return m_e.flat(data_offset() + i);
  }
  ...
  template <class CT, class... S> inline auto xview<CT, S...>::access() -> reference {
    return access_impl(make_index_sequence());
  }

  template <class CT, class... S> template <class Arg, class... Args>
  ...
  template <class CT, class... S> inline auto xview<CT, S...>::access() const -> const_reference {
    return access_impl(make_index_sequence());
  }
  ...
  inline auto xview<CT, S...>::make_index(It first, It last) const -> base_index_type {
    ...
    auto func1 = [&first](const auto& s) noexcept { return get_slice_value(s, first); };
    auto func2 = [](const auto& s) noexcept { return xt::value(s, 0); };
  ...
  template <class CT, class... S> inline auto xview<CT, S...>::compute_shape(std::true_type) const
      -> inner_shape_type {
    return inner_shape_type(m_e.shape());
  }

  template <class CT, class... S> inline auto xview<CT, S...>::compute_shape(std::false_type) const
      -> inner_shape_type {
  ...
  namespace xview_detail {
    ...
    inline void run_assign_temporary_impl(V& v, const T& t,
  ...
  inline void xview<CT, S...>::assign_temporary_impl(temporary_type&& tmp) {
    ...
    xview_detail::run_assign_temporary_impl(*this, tmp,
                                            std::integral_constant<bool, fast_assign>{});
  }

  namespace detail {
    template <class E, class... S> inline std::size_t get_underlying_shape_index(std::size_t I) {
      return I - newaxis_count_before<get_slice_type<E, S>...>(I);
    }

    template <class... S> struct check_slice;

    template <> struct check_slice<> {
      using type = void_t<>;
    };

    template <class S, class... SL> struct check_slice<S, SL...> {
    ...
    inline auto make_view_impl(E&& e, std::index_sequence<I...>, S&&... slices) {
      ...
      return view_type(
          std::forward<E>(e),
          get_slice_implementation(e, std::forward<S>(slices),
  ...
  template <class E, class... S> inline auto view(E&& e, S&&... slices) {
    return detail::make_view_impl(std::forward<E>(e), std::make_index_sequence<sizeof...(S)>(),
  ...
  namespace detail {
    class row_impl {
    public:
      template <class E> static inline auto make(E&& e, const std::ptrdiff_t index) {
    ...
    private:
      template <class S> static inline void check_dimension(const S& shape) {
        if (shape.size() != 2) {
          XTENSOR_THROW(std::invalid_argument,
   ...
   */
  template <class E> inline auto row(E&& e, std::ptrdiff_t index) {
   ...
   * @throws std::invalid_argument if the expression has more than 2 dimensions.
   */
  template <class E> inline auto col(E&& e, std::ptrdiff_t index) {
  ...
  template <class CT, class... S> template <class ST, bool Enable>
  ...
  inline void xview_stepper<is_const, CT, S...>::to_end_impl(layout_type l) {
    auto func = [](const auto& s) noexcept { return xt::value(s, get_size(s) - 1); };
  ...
  template <bool is_const, class CT, class... S> template <class F>
  ...
  void xview_stepper<is_const, CT, S...>::common_reset(size_type dim, F f, bool backwards) {
    auto size_func = [](const auto& s) noexcept { return get_size(s); };
    auto end_func
        = [](const auto& s) noexcept { return xt::value(s, get_size(s) - 1) - xt::value(s, 0); };
